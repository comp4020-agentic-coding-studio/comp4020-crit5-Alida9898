...
const BUDGET = {
  ...
  pool: 1,
  /** 终点大池另外加的九件:柱、盆、盆里的水,加上通关时喷出来的一道水柱和
   *  五颗水滴 —— 也就是双层喷泉的上层,和它喷起来的样子。
   *
   *  水滴数写在 `FORM.sprayDropCount`,这里是它的预算对应物:想多喷几颗,
   *  两个数都得动,而那正是「预算是决定,不是观察值」的意思 —— 一个粒子系统
   *  会让这条不变量彻底失效,所以喷泉是数得清的几颗,不是粒子。
   *
...
for (const { name, level, layout } of LEVELS) {
  describe(`${name} 的场景图守空间法则`, () => {
    ...
    it("开局喷泉不喷 —— 通关反馈不许在通关之前出现", () => {
      // §6:遮挡只允许发生在解开之后,玩家还在解的时候不许有任何东西盖住
      // 承重的接缝。喷出来的水是这一版唯一的通关反馈(原本规格写的是枝叶,
      // 换成喷泉是 2026-09-01 的决定,记在 CLAUDE.md §6),所以它同时也是
      // 唯一一件「解开之后才准出现」的东西。
      //
      // 这条是眼睛几乎抓不到的:开局那一屏喷泉如果就在喷,玩家不会觉得是 bug,
      // 只会不知道自己什么时候赢的。
      const { built } = stage(level, layout);
      for (const [id, s] of built.grandSpray) {
        expect(s.group.visible, `${id} 的喷泉开局就在喷`).toBe(false);
      }
      // 而且它只能长在 grand 的池子上,不是每口池子都喷。
      const grand = level.pools.filter((p) => p.grand).map((p) => p.id);
      expect([...built.grandSpray.keys()].sort()).toEqual(grand.sort());
    });
