...
- Everything is built from Three.js built-in primitives. **External 3D models
  ...
  **One exception, added 2026-09-02 at the author's direction: `public/backdrop.jpg`**,
  the top half of the author's reference image, sits *behind* the canvas at 40%
  opacity as the stage's backdrop. What the ban is protecting survives intact:
  no image is sampled by any material, nothing in the scene graph comes from a
  file, and every solid in the picture is still generated by code. The photo is
  atmosphere behind glass. Three conditions keep it that way, and a future
  backdrop has to meet them too:
  - **Behind the canvas, never on a material.** The renderer stays
    `alpha: true` and every material stays opaque, so §3.4 is untouched.
  - **Faint enough to lose.** All of this game's information is carried by the
    edges and faces of flat-coloured solids; a backdrop that competes for
    attention takes the puzzle with it. 40% is the number, and it lives in
    `styles.css` beside the rule.
  - **Never behind text.** It is scoped to `.stage`, so the title still sits on
    flat sandstone. CLAUDE.md's own warning applies: axe cannot resolve contrast
    over an image, in jsdom *or* in Chrome, so text over one would become a
    permanent hand-check. Not putting text there is cheaper than measuring it.
