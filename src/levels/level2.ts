...
// 教规则 3:**水只从兽脚下那口池子开始流**。
//
// 开局 315°,两处断口都断着:楼梯在 45° 接上出生点,拐角渠在 135° 接上。
// 右键一下上楼,再右键一下通水 —— 而第二次转视角落地时兽必须**已经站在蓄水池
// 上**,否则没有水源可言。
//
...
export const level2: Level = {
  ...
  // 开局 315°:**两处断口都是断的**。楼梯在 45° 接上,渠在 135° 接上,所以
  // 右键一下上楼、再右键一下通水。开局就能上楼的那一版没有解谜可言 —— 玩家
  // 第一眼看到的必须是「上不去」,否则「转一下」这个动作根本没有被要求过。
  opens: { camera: 315, turns: {} },
};
...
export const level2Layout: Layout = {
  ...
  ports: {
    ...
    // 楼梯的脚 (-1,0,-1) = birth 沿 45° 隐身方向**倒退**一格。倒退,不是前进:
    // (1,1,1) 在 45° 下是朝相机的方向,楼梯放在那一侧就比兽更近,深度缓冲判它
    // 在前,于是石阶盖住了兽。§3.4 不许动画序,所以「兽在上层」只能是楼梯**真的**
    // 更远。代价是出生点得抬到 y=1,楼梯的脚才不会掉到地面以下。
    //
    ...
    stair: { from: [-1, 0, -1], to: [-3, 1, -1] },

    cistern: { at: [-4, 1, -1] },
    cisternDeck: { at: [-4, 1, -1] },

    // 第一段:沿 +z 两格。
    chanA: { from: [-4, 1, -1], to: [-4, 1, 1] },

    elbow: { at: [-5, 2, 2] },
    elbowDeck: { at: [-5, 2, 2] },

    // 第二段:沿 -x 两格。方向和第一段不同,屏幕上因此是一个真正的 90° 拐角。
    chanB: { from: [-5, 2, 2], to: [-7, 2, 2] },
