...
export const INPUT = {
  ...
  rippleMs: 520,
  /** 点在**走不到**的地方时那圈涟漪的时长。同样的一圈,更短更淡 ——
   *  「收到了」和「这一下有用」是两件事,玩家得能分开。全长会读成
   *  「兽要出发了」,而它并不会出发。 */
  rippleMissMs: 300,
...
export const FORM = {
  ...
  /** 终点大池是**双层喷泉**:砖里那口内嵌的池子是下层,一根柱子托起一只浅盆
   *  是上层。规格 §5 把它叫「塔顶那口大蓄水池」,双层是给它一个和别的池子
   *  一眼分得开的形。
   *
   *  下层完全不变 —— 「水面低于池沿」那条不变量量的就是它,喷泉不许把它破坏。
   *  柱和盆是长在池子**上面**的东西,不参与那条判定。 */
  fountainStemRadius: 0.1 * TILE,
  fountainStemHeight: 0.34 * TILE,
  /** 盆比柱宽得多,才读得出是「盆」不是「柱头」;但要窄于池沿,否则它会
   *  把下层那口池子从上面盖死。 */
  fountainBowlRadius: 0.3 * TILE,
  fountainBowlHeight: 0.12 * TILE,
  /** 盆里那层水。它比盆沿低 `poolSink` —— 和下层那口池子用的是**同一个**常数,
   *  因为那是同一句话:「内嵌」就是水面低于沿口。 */
  fountainBowlWater: 0.04 * TILE,

  /** 通关的那一刻,喷泉开始喷水 —— 这是整个游戏唯一的通关反馈,所以它必须
   *  在玩家此刻正盯着的地方发生,也就是大池刚满的那口盆。
   *
   *  水柱和水滴都是**不透明**的实心 lapis(§3.4:水是实心青金色,连水都不许
   *  开透明),靠动起来说明它是水,不靠半透明。解开之前它们整组隐藏 ——
   *  §6 明令「只有解开之后才允许遮挡」。 */
  sprayJetRadius: 0.028 * TILE,
  sprayJetHeight: 0.44 * TILE,
  /** 几颗水滴。少而清楚,不是粒子系统 —— 每一颗都在 `BUDGET` 里算数。 */
  sprayDropCount: 5,
  sprayDropRadius: 0.045 * TILE,
  /** 水滴抛出去多远、抛多高,以及一个来回多久(毫秒)。 */
  sprayReach: 0.26 * TILE,
  sprayRise: 0.3 * TILE,
  sprayMs: 1100,
...
export const BEAST = {
  ...
  /** 走路时上下起伏的幅度。 */
  bob: 0.03 * TILE,
  /** 挪向目标的跟随速度(每秒趋近的比例)。以前是 scene.ts 里写死的 7,
   *  §4 说可调参数一个都不许散在逻辑里。 */
  stepRate: 7,
  /** 转头的角速度跟随比例。比走路快 —— 转头是即时的回执,走路是过程。 */
  yawRate: 12,
} as const;
