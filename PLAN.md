...
| | 之前建的 | 规格要求 |
|---|---|---|
| 玩家转的是 | **露台**,相机不动 | 规格写相机;**当前决定:转砖块,相机固定** |
| 连通表按什么索引 | 每块露台转到第几档 | 统一的 `when`,两者通吃 |
...
规格原本要求玩家转相机。**当前决定:相机固定在 45°,玩家转砖块。**

为了让这个决定可以随时反悔而不重写:一条声明的生效条件是
`when: { camera?, turns? }`,省略即「不关心」。现在所有声明只写 `turns`。
将来要把相机转回来,给声明多填一个 `camera` 即可 —— `rules.ts` 和全部传感器
一行都不用改。这是这次改动唯一的设计成本,值得付。
